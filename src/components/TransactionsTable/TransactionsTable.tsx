...
export default function TransactionsTable({ transactions }: Props) {
  ...
  const descriptionCellRenderer = (rowIndex: number) => <Cell>{transactions[rowIndex].description}</Cell>;
  const dateCellRenderer = (rowIndex: number) => <Cell style={{ textAlign: "center" }}>{formatDate(transactions[rowIndex].date)}</Cell>;
  const categoryCellRenderer = (rowIndex: number) => <Cell style={{ textAlign: "center" }}>{transactions[rowIndex].category_name}</Cell>;
  ...
  const notesCellRenderer = (rowIndex: number) => <Cell>{transactions[rowIndex].notes ?? ""}</Cell>;
  ...
  return (<>
    ...
    <Table numRows={transactions.length} cellRendererDependencies={transactions}>
      {columns.map((col, idx) => (
        <Column key={idx} name={col.name} cellRenderer={col.cellRenderer} columnHeaderCellRenderer={headerRowRenderer} />
      ))}
    </Table> </>)
}
